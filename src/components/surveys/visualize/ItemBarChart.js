...
const ItemBarChart = ({ stats }) => {
    const chartData = Object.entries(invertFilename2Frequency(stats)).map(([filename, count]) => ({
        filename,
        count
    }));
    ...
    return (
        <div>
            <BarChart width={500} height={400} data={chartData} margin={{ top: 20, right: 30, bottom: 30, left: 20 }}>
                ...
                <XAxis dataKey="filename" label={{ value: 'Survey Items', position: 'insideBottomRight', dy: 10 }} />
